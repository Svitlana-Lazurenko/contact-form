...
function handle_custom_contact_form_ajax()
{
    ...
    // $dataGoogleSheets = [$name, $phone, $email, $description, $utmSource, $utmMedium, $utmCampaign, $utmTerm, $utmContent, $formDate];
    ...
    // ======================== Sending a data for Google Sheets ====================

    // function send_to_google_sheets($dataGoogleSheets)
    // {
    //     $spreadsheetId = get_option('google_sheets_id');
    //     $spreadsheetKey = get_option('google_sheets_key');

    //     $client = new Google_Client();
    //     $client->setDeveloperKey($spreadsheetKey);
    //     $client->setScopes(['https://www.googleapis.com/auth/spreadsheets']);

    //     $service = new Google_Service_Sheets($client);
    //     $range = '';
    //     $body = new Google_Service_Sheets_ValueRange([
    //         'values' => [$dataGoogleSheets]
    //     ]);
    ...
    //             $params
    //         );
    //         if (!$response || $response->getUpdates()->getUpdatedRows() === 0) {
    //             wp_send_json_error('Помилка відправки в Google Sheets.', 500);
    //         }
    //     } else {
    //         return;
    //     }
    ...
    // send_to_google_sheets($dataGoogleSheets);
